...
- Adjust/add the names of MINFLUX excitation lasers, activation lasers, and detectors in the the lists in hardware_settings/mfx_exc_lasers, mfx_act_lasers, mfx_detectors_gui mfx_detectors_imspector, and mfx_detectors_imspector_threads in etMINFLUX_setup.json. The names in mfx_exc_lasers, mfx_act_lasers, and mfx_detectors_gui have to match the corresponding names in the Imspector GUI (for detectors, this is single detectors or combination of detectors). The names/lists in mfx_detectors_imspector are backend IDs in Imspector for the detectors, and are named as DETX or APDX (depending on the system) where X is an ID starting at 1, for the detector that is on top of the list of the GUI, and increases by one for each subsequent detector in the list of detectors of the GUI. The default in etMINFLUX_setup.json is DET, but if your system uses the APD naming convention all DET mentions have to be replaced with APD (such as DET2 -> APD2). You can find out which naming convention your system uses with the specpy command imspector.value_at('ExpControl/measurement/channels/0/detsel/detector', specpy.ValueTree.Measurement).get() command after imspector has been initiated as imspector = specpy.get_application(); alternatively, if the DET convention does not work (it will always select the top detector, normally DAPI, no matter which one you choose), change to the APD convention. The names/lists in mfx_detectors_imspector_threads are yet another naming used in the acquisition threads functionality in Imspector, and this list should match the mfx_detectors_imspector list with the difference being that in mfx_detectors_imspector_threads even the single entries (such as DET1 in the example .json provided) should be list elements in a string ("DET1" -> "[\"DET\"]" or "APD1" -> "[\"APD\"]"). See the example .json and follow the same pattern, or leave untouched if your system has the same detectors available.
