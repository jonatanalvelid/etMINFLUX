...
def gag_signalrise(img, prev_frames=None, binary_mask=None, exinfo=None, presetROIsize=None,
                     min_dist=1, num_peaks=500, thresh_abs_lo=1.5, thresh_abs_hi=300, border_limit=10,
                     memory_frames=5, track_search_dist=10, frames_appear=3, thresh_intincratio=1.1,
                     thresh_intincratio_max=5, thresh_move_dist=3):
    """
    Common parameters:
    img - current image,
    ...
    smoothing_radius_raw = 1  # pixels
    intensity_sum_rad = 2  # pixels
    frames_appear = int(frames_appear)
    memory_frames = int(memory_frames)
    meanlen = int(np.min([3, frames_appear]))
    track_len = int(2 * frames_appear + 1)  # number of last frames to keep in the tracking (more frames = slower track linking)
    track_search_dist = int(track_search_dist)
    thresh_stayframes = int(frames_appear-1)  # can only be gone for 1 frame after
    
    if binary_mask is None:
        binary_mask = np.ones(np.shape(img)).astype('uint16')
    
    img = np.array(img).astype('float32')
    img = ndi.gaussian_filter(img, smoothing_radius_raw)
            ...
            for _, track in tracks_timepoint.iterrows():
                ...
                if particle_id not in particle_ids_before:
                    ...
                    if len(track_self_after) > thresh_stayframes:
                        ...
                        int_init = np.mean(track_intensity_after.iloc[:meanlen])
                        int_last = np.mean(track_intensity_after.iloc[-meanlen:])
                        intincratio_before = int_init/int_before
                        intincratio = int_last/int_init
                        intincrratio_tot = int_last/int_before
                        if (intincratio_before > thresh_intincratio or intincratio > thresh_intincratio or intincrratio_tot > thresh_intincratio) and intincratio_before < thresh_intincratio_max:
                            ...
                            if d_start_end < thresh_move_dist:
                                ...
                                if int(track_self['x']) > border_limit and int(track_self['x']) < imgsize - border_limit and int(track_self['y']) > border_limit and int(track_self['y']) < imgsize - border_limit:
                                    ...
                                    print([track_intensity_before[-meanlen:], track_intensity_after])
