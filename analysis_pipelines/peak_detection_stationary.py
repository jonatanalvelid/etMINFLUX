...
def peak_detection_stationary(img, prev_frames=None, binary_mask=None, exinfo=None, presetROIsize=None,
                       maxfilter_kersize=5, peak_min_dist=7, thresh_abs=10, smoothing_radius=1, 
                       border_limit=15, init_smooth=1, roi_border=3, roi_th_factor=6, num_prev=4, msm_thresh=0.7):
    """
    Common parameters:
    img - current image,
        ...
        for particle in np.unique(tracks_use['particle']):
            ...
            if len(particle_track) > num_prev:
                ...
                if np.mean(sm) < msm_thresh:
                    print(f'EVENT, frame {timepoint}')
                    coords_events.append(np.array([int(particle_track.iloc[-1]['x']), int(particle_track.iloc[-1]['y'])]))
                    print(coords_events)
                    print('')
                    #break
    ...
    return coords_events, roi_sizes, img_ana, tracks_all