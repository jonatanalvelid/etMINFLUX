...
def peak_detection_def(img_ch1, prev_frames=None, binary_mask=None, exinfo=None, presetROIsize=None,
    ...
    if len(coordinates) > int(coord_num):
        # make sure enough coords are detected, larger than defined coordinate number
        coordinates = np.array([coordinates[int(coord_num)]])
    else:
        # if not, get last coordinate
